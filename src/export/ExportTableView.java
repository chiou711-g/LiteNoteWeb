...
@WebServlet("/export/ExportTableView.do")
public class ExportTableView extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// get table view file name
		String fileName =  request.getParameter("tableViewFileName");
			...
			try {
				...
				while (rSet.next()) 
				{
					...
					if(Integer.valueOf(category_pos_selection[i]) == row_count)
					{
						...
						System.out.println("ExportTableView / _doPost / category_selection[" + i +"] = " + category_selection[i]);
						break;
					}
				} 
		...
		System.out.println("ExportTableView / _doPost / categoriesCount = " + categoriesCount);
		System.out.println("ExportTableView / _doPost / pagesCount = " + pagesCount);
		...
		if(category_selection != null)
		{
			...
			{
				System.out.print("ExportTableView / _doPost / categories: ");
				...
				for(int p=0;p<pageSelCnt;p++)
				{
					...
					try {			
						while(rs.next())
						{
							...
							System.out.println("ExportTableView / _doPost / image_uri = " + image_uri);
							if(image_uri != null)
					...
					if(category_id == Integer.valueOf(category_selection[c]))
					{
						...
						pages.put(page);
						System.out.println("ExportTableView / _doPost / add categoryName = " + categoryName);
					}
				}
	    ...
	    for(int i=0;i<categories.length();i++  )
	    {
	    	...
	    	for(int j=0;j<pages.length();j++)
	    	{
    			...
    			for(int k=0;k<links.length();k++)
	    		{
	    			...
	    			listStr = listStr.concat("<img src=" + pictureUri + " style=\"width:206px\">");
		    	...
		    	listStr = listStr.concat("</div>");
		    	listStr = listStr.concat("<br>");
	    	}
 	    }
	    System.out.println("ExportTableView / listStr = " + listStr);	
	    
		// copy path
		String copyPath = null;

		// save GDrive table view file (keep release track: table view 
	    ...
	    writer.write(getHtmlString(listStr));
	    writer.close();
		...
		{
	    	copyPath = "./Java-workspace/LiteNoteWeb/WebContent/EXPORT_TABLE_VIEW.html";
			BufferedWriter writer2 = getWriter(copyPath);
		    writer2.write(getHtmlString(listStr));
		    writer2.close();		
		}
		System.out.println("ExportTableView / _doPost / copyPath = " + copyPath);
